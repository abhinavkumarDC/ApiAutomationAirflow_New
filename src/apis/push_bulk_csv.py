...
class PushBulkCsv:
    def __init__(self, config, token):
        ...
        self.input_csv_file_path = os.path.join(self.input_dir, 'common_push_bulk_csv.csv')

    def fetch_and_store_get_notification_data(self):
        data = self.helpers.fetch_data_from_api(self.fetch_api_url, self.token)
        if data:
            json_filename = os.path.join(self.output_dir, 'pushBulkCsvData.json')
            csv_filename = os.path.join(self.output_dir, 'pushBulkCsvData.csv')
